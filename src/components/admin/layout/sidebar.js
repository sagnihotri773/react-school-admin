...
import { Package2Icon, HomeIcon, LineChartIcon, PackageIcon, ShoppingCartIcon, UsersIcon, ChevronRightIcon, isMobile } from '../../../utils/utils'
...
const Sidebar = () => {
  ...
  return (
    ...
    <div className={`bg-gray-900 text-white flex flex-col gap-4 ${isMobile && 'customHide'}`}>
      <div className="flex flex-col gap-2">
        ...
        <div className="flex-1">
          <nav className="grid items-start text-sm font-medium">
            {/* {menuItems.map((x) => (
              <Link
                className={`flex items-center gap-2 rounded-md px-3 py-2 hover:bg-gray-800 transition-colors ${routeName === x.link ? "" : ""}`}
