...
import { adminStudentDetails, adminFeeDetail } from './json'
                    ...
                    <main className="flex flex-1 flex-col gap-4 p-4 md:gap-8 md:p-6">
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {adminStudentDetails?.map((cardValue, i) => (
                                <Card key={i} className={`dark:bg-[${cardValue.color}] bg-[${cardValue.color}] text-white`}>
                        ...
                        <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                            {adminFeeDetail?.map((cardValue, i) => (
                                <Card key={i} className={`bg-[${cardValue.cardColor}] dark:bg-[${cardValue.cardColor}]`}>
                                    <CardContent className="flex p-6 items-center gap-4">
                                        <div className={`bg-[${cardValue.symbolBG}] dark:bg-[${cardValue.symbolBG}] rounded-full p-3`}>
